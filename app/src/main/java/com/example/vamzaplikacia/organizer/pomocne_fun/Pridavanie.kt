 ...
 */
fun pridajZadanuKnihu(uiState: FormularKnihyUIState, kniznica: Kniznica, context: Context): Kniha {
    ...
    val cesta: Uri? = if (uiState.obrazok != null) {
        ulozObrazokDoInternehoUloziska(context, uiState.obrazok, uiState.nazov.replace(' ', '_'))
    } else {
        null
    }

    val kniha = Kniha(uiState.nazov, uiState.autor, rok, uiState.vydavatelstvo,
        cesta, uiState.popis, uiState.poznamky, uiState.precitana, uiState.naNeskor,
 ...
 * @param kniznica knižnica aplikácie
 */
fun pridajZadanehoAutora(uiState: FormularAutorUIState, kniznica: Kniznica, context: Context): Autor {
    val cesta: Uri? = if (uiState.obrazok != null) {
        ulozObrazokDoInternehoUloziska(context, uiState.obrazok, uiState.menoAutora.replace(' ', '_'))
    } else {
        null
    }
    val autor = Autor(uiState.menoAutora, uiState.datumNar, uiState.datumUmrtia, obrazokCesta = cesta)
 ...
 * @param kniznica
 */
fun pridajZoznam(nazov: String, obrazok: Uri? = null, kniznica: Kniznica, context: Context): ZoznamKnih {
    val cesta: Uri? = if (obrazok != null) {
        ulozObrazokDoInternehoUloziska(context, obrazok, nazov.replace(' ', '_'))
    } else {
        null
    }
    var zoznam = ZoznamKnih(nazov, obrazok = cesta)
